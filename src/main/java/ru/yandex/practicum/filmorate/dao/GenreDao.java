...
public class GenreDao implements GenreStorage {
    ...
    public void updateGenresOfFilm(Film film) {
        String removeFilmGenre = "DELETE FROM FilmGenres where film_id = ?";
        jdbcTemplate.update(removeFilmGenre, film.getId());

        String addFilmGenre = "MERGE INTO FilmGenres (film_id, genre_id) VALUES (?, ?)";
    ...
    public List<Genre> findGenresForFilm(int filmId) {
        String genres = "SELECT g.genre_id genre_id, g.genre_name genre_name " +
                "FROM FilmGenres fg " +
                "INNER JOIN Genres g " +
